...
def test_network(target):
    ...
    i = 0
    assert len(
        target.layers) == 3, f"Wrong number of layers. Expected 3 but got {len(target.layers)}"
...
def test_optimizer(target, ALPHA):
    assert type(
        target) == Adam, f"Wrong optimizer. Expected: {Adam}, got: {target}"
    assert np.isclose(target.learning_rate.numpy(
    ), ALPHA), f"Wrong alpha. Expected: {ALPHA}, got: {target.learning_rate.numpy()}"
    print("\033[92mAll tests passed!")
...
def test_compute_loss(target):
    ...
    loss = target((states, actions, rewards, next_states, done_vals),
                  0.995, q_network_random, target_q_network_random)

    assert np.isclose(
        loss, 0.6991737), f"Wrong value. Expected {0.6991737}, got {loss}"
    ...
    assert np.isclose(
        loss, 0.343270182), f"Wrong value. Expected {0.343270182}, got {loss}"
